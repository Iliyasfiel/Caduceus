...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def share(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            data['share_token'] = str(task.share_token)
    ...
    @action(
        ...
        url_path='share/(?P<token>[^/.]+)',
        permission_classes=[], authentication_classes=[]
    )
    def public_share(self, request, token=None):
        """
        公开分享查看 - 无需登录
        通过 share_token UUID 获取任务基本信息、管线进度和公开字段
        """
        task = get_object_or_404(Task, share_token=token)
        if task.share_expires_at and task.share_expires_at < timezone.now():
            return Response({'detail': '分享已过期'}, status=status.HTTP_410_GONE)
        public_fields = [f for f in task.fields if f.get('is_public')]
        serializer = TaskSerializer(task)
        data = serializer.data
        data['fields'] = public_fields
        return Response(data)
