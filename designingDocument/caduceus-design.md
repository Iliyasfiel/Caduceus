...
- `name: str` — 如"酒店"、"餐厅"、"车辆"
- `icon: str` — 前端展示图标
- `schema: JSONB` — 定义该资源类型的标准字段结构
  - 例：`[{key:"star_level", label:"星级", type:"select", options:["3星","4星","5星"]}, {key:"address", label:"地址", type:"text"}]`

**ResourceItem**（资源条目）
- `resource_type: FK → ResourceType`
- `name: str`
- `fields: JSONB` — 存储 schema 定义的字段值
  - 例：`{"star_level":"5星", "address":"西湖路1号"}`
- `extra_fields: JSONB` — 用户自定义扩展键值对
  - 例：`{"有无停车场":"有", "WiFi密码":"xxxx", "对接人":"张三"}`
- `available: boolean` — 是否可被预约

**ResourceBooking**（资源预约/占用）
