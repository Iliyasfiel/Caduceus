...
| `UiSelect` | v-model / options: `[{label, value}]` 或 `[['label', value]]` / label / error / disabled / required | ✅ | 与规划一致 |
...
> 组件完整 Props / Emits / Slots API 见 [dev-tmp/ui-component-api-reference.md](./dev-tmp/ui-component-api-reference.md)（API 参考文档，按规则放入 dev-tmp/，清理时需经用户确认是否同步沉淀）。
